...
public class OCRApplet extends Applet
{
  ...
  public void init()
  {
    ...
    topButtonPanel.add(recognize = new Button("Rekognisi"), BorderLayout.SOUTH);
    topButtonPanel.add(clear = new Button("Bersihkan"));
    Panel addPanel = new Panel();
    addPanel.setLayout(new GridLayout(1, 2));
    addPanel.add(add = new Button("Tambah:"));
    ...
    bottomButtonPanel.add(del = new Button("Hapus"));
    bottomButtonPanel.add(delAll = new Button("Hapus Semua"));
    bottomButtonPanel.add(train = new Button("Latih"));

    // create top panel
    entry = new Entry();
    topPanel.setLayout(new BorderLayout());
    topPanel.add(message = new Label("Gambar, kemudian click Rekognisi"),
        BorderLayout.NORTH);
    ...
    bottomPanel.add(new Label(" "), BorderLayout.NORTH);
  ...
  TextField letterToAdd = new TextField("", 1);
    ...
    private static class KohonenNetwork {

        public KohonenNetwork(int inputNeuron, int outputNeuron, OCRApplet aThis) {
        }

        private void setTrainingSet(TrainingSet set) {
            throw new UnsupportedOperationException("Tidak mendukung"); //To change body of generated methods, choose Tools | Templates.
        }
  ...
  void add_actionPerformed(java.awt.event.ActionEvent event)
  {
    ...
    if (letter.length() > 1)
    {
      message.setText("Hanya Satu huruf");
      return;
    }

    if (letter.length() < 1)
    {
      message.setText("Masukan satu huruf");
      return;
    }
    ...
    for (i = 0; i < sampleList.size(); i++)
    {
      ...
      {
        message.setText("Kata sudah ada");
        return;
      }
  ...
  void del_actionPerformed(java.awt.event.ActionEvent event)
  {
    ...
    if (i == -1)
    {
      message.setText("mohon pilih satu huruf untuk dihapus");
  ...
  void train_actionPerformed(java.awt.event.ActionEvent event)
  {
    ...
    try
    {
      ...
      message.setText("Siap untuk direkognisi.");
    } catch (Exception e)
    {
  ...
  void recognize_actionPerformed(java.awt.event.ActionEvent event)
  {
    if (net == null)
    {
      message.setText("silahkan training terlebih dahulu");
      return;
    }
    entry.downSample();
